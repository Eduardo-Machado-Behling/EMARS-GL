...
public class GameStationOpenGL extends AbstractMarsToolAndApplication {
  ...
  private final Queue<javafx.util.Pair<KeyEvent, KeyType>> buffer =
      new ArrayDeque<>();
  private final Set<Integer> pressedKeys =
      new HashSet<>(); // Track pressed keys
  ...
  static final int OPENGL_WRITABLE_DATA_AMOUNT =
      openGLHighAddress - openGLBaseAddress;
   ...
   */
  public String getName() { return "Game Station GPU"; }
  ...
  protected void processMIPSUpdate(Observable memory,
    ...
    if (accessNotice.getAccessType() == AccessNotice.WRITE) {
      ...
      case openGLRedrawAddress:
        try {
          canvas.drawCall(
              Memory.getInstance().getWordNoNotify(openGLBaseAddress));
  ...
  protected JComponent buildMainDisplayArea() {
    ...
    GLProfile glp = GLProfile.get(
        GLProfile.GL3); // Or try GLProfile.get(GLProfile.GL3) if needed
    GLCapabilities caps = new GLCapabilities(glp);
    canvas = new PixelBufferCanvas(
        width, height, 60, caps); // Pass capabilities to the constructor
  ...
  private void setupKeyBindings(JPanel panel) {
    ...
    int[] keys = {
        ...
        KeyEvent.VK_SPACE,  KeyEvent.VK_ENTER,   KeyEvent.VK_BACK_SPACE,
        KeyEvent.VK_TAB,    KeyEvent.VK_SHIFT,   KeyEvent.VK_CONTROL,
        KeyEvent.VK_ALT,    KeyEvent.VK_UP,      KeyEvent.VK_DOWN,
        KeyEvent.VK_LEFT,   KeyEvent.VK_RIGHT,   KeyEvent.VK_ESCAPE,
        KeyEvent.VK_DELETE, KeyEvent.VK_INSERT,  KeyEvent.VK_HOME,
  ...
  private void bindKey(JPanel panel, int keyCode, KeyType type,
                       boolean pressed) {
    String actionName =
        "key_" + keyCode + "_" + (pressed ? "pressed" : "released");
    ...
    actionMap.put(actionName, new AbstractAction() {
      ...
      public void actionPerformed(ActionEvent e) {
        KeyEvent keyEvent = new KeyEvent(
            panel, pressed ? KeyEvent.KEY_PRESSED : KeyEvent.KEY_RELEASED,
  ...
  public class FPSTask extends TimerTask {
    ...
    FPSTask(GameStationOpenGL parent) { this.parent = parent; }
  ...
  protected class PixelBufferCanvas
    ...
    public void display(GLAutoDrawable drawable) {
      ...
      if (frameCount % 60 == 0) { // Log every 60 frames
        // System.out.println( "Frame " + frameCount + " - Canvas size: " +
    ...
    private void updateBuffer(GL3 gl) {
      gl.glBindBuffer(GL3.GL_ARRAY_BUFFER, instanceId[0]);
      buffer = gl.glMapBuffer(GL3.GL_ARRAY_BUFFER,
                              GL3.GL_MAP_WRITE_BIT | GL3.GL_READ_WRITE);
      System.out.println("redraw: " + instanceAmount);
    ...
    public void dispose(GLAutoDrawable drawable) {
      ...
      gl.glDeleteBuffers(1, vertexBufferId,
                         0); // if you need to delete the vertex buffer
    ...
    private void createVertexBuffer(GL3 gl) {
      ...
      gl.glBufferData(GL3.GL_ARRAY_BUFFER, vertices.length * Float.BYTES,
                      vertexBufferData, GL3.GL_STATIC_DRAW);
    ...
    private void createInstanceBuffer(GL3 gl) {
      ...
      gl.glBufferData(GL3.GL_ARRAY_BUFFER,
                      GameStationOpenGL.OPENGL_WRITABLE_DATA_AMOUNT, null,
                      GL3.GL_DYNAMIC_DRAW);
    ...
    private class ShadersUtils {
      ...
      public void init(GL3 gl) {
        vertexShader =
            createShader(gl, GL3.GL_VERTEX_SHADER, defaultVertexShader);
        int colorFrag =
            createShader(gl, GL3.GL_FRAGMENT_SHADER, defaultFragShader);
        ...
        if (compiled.get(0) == GL3.GL_FALSE) {
          ...
          if (logLength.get(0) > 0) {
            ByteBuffer logBuffer =
                Buffers.newDirectByteBuffer(logLength.get(0));
      ...
      private int createProgramId(GL3 gl, int vertexShader,
                                  int fragmentShader) {
        ...
        if (linked.get(0) == GL3.GL_FALSE) {
          ...
          if (logLength[0] > 0) {
            ...
            gl.glGetProgramInfoLog(program, logLength[0], logLength, 0,
                                   logBytes, 0);
    ...
    private class TextureManager {
      ...
      private TextureManager() {
        ...
        this.bitmapsToLoad =
            new ArrayDeque<>(); // Use a LinkedList or ArrayDeque
      ...
      public void init(GL3 gl) {
        ...
        gl.glBindTexture(GL3.GL_TEXTURE_2D_ARRAY,
                         glTextureArray); // Bind as texture array

        // Texture array parameters
        gl.glTexParameteri(GL3.GL_TEXTURE_2D_ARRAY, GL3.GL_TEXTURE_WRAP_S,
                           GL3.GL_REPEAT);
        gl.glTexParameteri(GL3.GL_TEXTURE_2D_ARRAY, GL3.GL_TEXTURE_WRAP_T,
                           GL3.GL_REPEAT);
        gl.glTexParameteri(GL3.GL_TEXTURE_2D_ARRAY, GL3.GL_TEXTURE_MIN_FILTER,
                           GL3.GL_NEAREST);
        gl.glTexParameteri(GL3.GL_TEXTURE_2D_ARRAY, GL3.GL_TEXTURE_MAG_FILTER,
                           GL3.GL_NEAREST);
        ...
        System.out.println("Max texture array layers: " +
                           maxArrayLayers.get(0));
        int depth = Math.min(GameStationOpenGL.OPENGL_WRITABLE_DATA_AMOUNT / 6,
                             maxArrayLayers.get(0));
        gl.glTexStorage3D(GL3.GL_TEXTURE_2D_ARRAY, 1, GL3.GL_RGBA8,
      ...
      public void register(int address) {
        ...
        try {
          ...
          for (int i = 0; i < textureWidth * textureHeight; i++) {
            ...
            y -= textureHeight - height;

            if (x < width && y < height && y >= 0) {
              int addr = (reverseAddr - (y + 1) * width * 4) + x * 4;
              value = Memory.getInstance().getWordNoNotify(addr);
            }
            bb.putInt(value);
          }
      ...
      public void populateTexture(GL3 gl) {
        ...
        while (!bitmapsToLoad.isEmpty()) {
          ...
          gl.glTexSubImage3D(GL3.GL_TEXTURE_2D_ARRAY, 0, 0, 0, texturesLoaded,
                             textureWidth, textureHeight, 1, GL3.GL_RGBA,
                             GL3.GL_UNSIGNED_BYTE, val.data);
